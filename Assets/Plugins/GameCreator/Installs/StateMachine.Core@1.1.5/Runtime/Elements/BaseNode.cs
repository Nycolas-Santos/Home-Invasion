...
namespace NinjutsuGames.StateMachine.Runtime
{
    ...
    public abstract class BaseNode
    {
        ...
        public virtual bool hideControls => false; 
        ...
        /// Node enabled state
        /// </summary>
        public bool enabledForExecution = true;
        ...
        [NonSerialized] public Dictionary<string, NodeFieldInformation> nodeFields = new();
        ...
        [NonSerialized] protected StateMachineAsset graph;
        ...
        public bool UpdatePortsForFieldLocal(string fieldName, bool sendPortUpdatedEvent = true)
        {
            ...
            void AddPortData(PortData portData)
            {
                ...
                else
                {
                    ...
                    if (!StateMachineAsset.TypesAreConnectable(port.portData.displayType, portData.displayType))
                    {
                    ...
                    if (node.UpdatePortsForFieldLocal(field, sendPortUpdatedEvent))
                    {
                        foreach (var port in node.IsFieldInput(field) ? (NodePortContainer) node.inputPorts : node.outputPorts)
                        {
                            ...
                            foreach (var edge in port.GetEdges())
                            {
                                ...
                                var fieldsWithBehavior = edgeNode.nodeFields.Values.Where(HasCustomBehavior).Select(f => f.fieldName).ToList();
        ...
        HashSet<BaseNode> portUpdateHashSet = new();
        ...
        public void OnProcess(GameObject context, Args customArgs = null)
        {
            // inputPorts.PullDatas();

            Process(context, customArgs);
        ...
        /// </summary>
        protected virtual void Process(GameObject context, Args customArgs = null)
        {
        }
