...
namespace NinjutsuGames.StateMachine.Editor
{
    ...
    public class BaseNodeView : NodeView
    {
        ...
        private Label computeOrderLabel = new Label();
        ...
        [NonSerialized] private List<IconBadge> badges = new List<IconBadge>();
        ...
        public static readonly TIcon ICON_LOCK = new IconLock(ColorTheme.Type.White);
        public static readonly TIcon ICON_DISABLE = new IconVisibleOff(ColorTheme.Type.White);
        ...
        public void Initialize(BaseGraphView owner, BaseNode node)
        {
            ...
            {
                var customStyle = Resources.Load<StyleSheet>(node.layoutStyle);
            ...
            // RegisterCallback<DragEnterEvent>(DragEnterCallback);
            // RegisterCallback<DragLeaveEvent>(DragLeaveCallback);
            // RegisterCallback<DragExitedEvent>(DragExitedCallback);
            OnGeometryChanged(null);
        }

        /*private void DragPerformedCallback(DragPerformEvent evt)
        {
            Debug.Log($"NODE Drag performed on {nodeTarget.name} {evt.target} data: {DragAndDrop.GetGenericData("GameCreator.Editor.Common.PolymorphicItemManipulator+DragData")}");
            DragAndDrop.AcceptDrag();

        }
        private void DragExitedCallback(DragExitedEvent evt)
        {
            Debug.Log($"DragExitedCallback {evt.target} data: {DragAndDrop.GetGenericData("GameCreator.Editor.Common.PolymorphicItemManipulator+DragData")}");
        }

        private void DragLeaveCallback(DragLeaveEvent evt)
        {
            Debug.Log($"DragEnterCallback {evt.target} data: {DragAndDrop.GetGenericData("GameCreator.Editor.Common.PolymorphicItemManipulator+DragData")}");
        }
        ...
        public void UpdateExecutionStateView()
        {
        ...
        protected void UnHighlight(bool runningResult)
        {
        ...
        private Dictionary<string, List<(object value, VisualElement target)>> visibleConditions = new Dictionary<string, List<(object value, VisualElement target)>>();
        private Dictionary<string, VisualElement> hideElementIfConnected = new Dictionary<string, VisualElement>();
        private Dictionary<FieldInfo, List<VisualElement>> fieldControlsMap = new Dictionary<FieldInfo, List<VisualElement>>();
        ...
        private Regex s_ReplaceNodeIndexPropertyPath = new Regex(@"(^nodes.Array.data\[)(\d+)(\])");
        ...
        public void UpdateLockStatusView()
        {
            if (nodeTarget.nodeLock)
            {
                ...
                mainContainer.parent.Add(lockIcon);
            }
            else
            {
                if(lockIcon != null)
                {
                    mainContainer.parent.Remove(lockIcon);
                }
            }
