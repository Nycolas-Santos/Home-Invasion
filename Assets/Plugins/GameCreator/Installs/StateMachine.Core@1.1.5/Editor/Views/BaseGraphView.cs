...
namespace NinjutsuGames.StateMachine.Editor
{
    ...
    public class BaseGraphView : GraphView, IDisposable
    {
        ...
        private Dictionary<Type, PinnedElementView> pinnedElements = new Dictionary<Type, PinnedElementView>();
        ...
        private Dictionary<Type, (Type nodeType, MethodInfo initalizeNodeFromObject)> nodeTypePerCreateAssetType = new Dictionary<Type, (Type, MethodInfo)>();
        /*private SortedDictionary<Type, (Type nodeType, MethodInfo initalizeNodeFromObject)> nodeTypePerCreateAssetType = new SortedDictionary<Type, (Type, MethodInfo)>(
        ...
        private List<BaseNodeView> elementsInGroup = new List<BaseNodeView>();
        private List<GroupView> groupsToUngroup = new List<GroupView>();
        ...
        {
            Debug.Log($"GRAPH DragExitedCallback {evt.target} data: {DragAndDrop.GetGenericData("DragData")}");
        }

        private void DragLeaveCallback(DragLeaveEvent evt)
        {
            Debug.Log($"GRAPH DragEnterCallback {evt.target} data: {DragAndDrop.GetGenericData("DragData")}");
        }
        ...
        protected virtual NodeInspector CreateNodeInspectorObject()
        {
        ...
        {
            get { return selection.Any(e => e is (BaseNodeView or GroupView) and not StartNodeView); }
        }
...
#if UNITY_2020_1_OR_NEWER
            Vector2 position = (evt.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer, evt.localMousePosition);
            evt.menu.InsertAction(menuPosition, "Create Sticky Note", e => AddStickyNote(new NinjutsuGames.StateMachine.Runtime.StickyNote("Create Note", position)), DropdownMenuAction.AlwaysEnabled);
#endif
        }
        ...
        protected virtual void KeyDownCallback(KeyDownEvent e)
        {
            ...
            else if (selection.Count > 0 && e.commandKey && e.keyCode == KeyCode.G)
            {
                ...
                else if(selection.Any(x => x is BaseNodeView))
                {
                    var allNodeViews = selection.FindAll(x => x is BaseNodeView);
                    var firstNode = allNodeViews[0] as BaseNodeView;
                    AddSelectionsToGroup(AddGroup(new Group($"{firstNode.nodeTarget.GetCustomName()} Group", firstNode.GetGlobalCenter())));
                    e.StopPropagation();
                }
            }
        ...
        public void Initialize(StateMachineAsset graph)
        {
            if (this.graph != null)
            {
                SaveGraphToDisk();
            ...
            // If this is a new graph, we add a start node
            if (graph.nodes.Count != 0) return;
            
            var pos = new Vector2(100, 300);
            var node = BaseNode.CreateFromType<StartNode>(pos);
            AddNode(node);
        ...
        private void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            {
                EditorApplication.delayCall += () => Initialize(graph);
            }
        }
