...
namespace NinjutsuGames.StateMachine.Editor
{
    ...
    public class StateMachineRunnerEditor : UnityEditor.Editor
    {
        ...
        {
            // Debug.Log($"OnAssetVariableChange");
            if(fieldRunnerList == null) return;
            SyncVariables(true, true);
        }
        ...
        private void SyncStateMachineVariables()
        {
            ...
            runner.stateMachineAsset.SyncVariables(runner.Runtime.List);
            EditorApplication.delayCall -= SyncStateMachineVariables;
        }
        ...
        private void SyncVariables(bool force = false, bool remove = false)
        {
            if(EditorApplication.isPlayingOrWillChangePlaymode) return;
            if(fieldRunnerList == null) return;
            ...
            for (var i = 0; i < runner.stateMachineAsset.NameList.Names.Length; i++)
            {
                ...
                if(exists)
                {
                    if (runner.stateMachineAsset.NameList.Get(i).Type != runner.Runtime.List.Get(i).Type)
                    {
                        // Debug.LogWarning($"Variable {nameVar} has different type in State Machine and Runtime. Runtime will be updated to State Machine type.");
